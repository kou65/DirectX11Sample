...
void Light::SetParameter(
	...
	m_light_data.LightColor = XMFLOAT4(1.f, 0.5f, 1.f, 1.f);
	...
	m_light_data.LightPos = XMFLOAT4(1.f, 1.f, 0.f, 1.f);
	m_light_data.MaterialAmbient = XMFLOAT4(0.5f, 0.5f, 0.5f, 1.f);
	m_light_data.MaterialDiffuse = XMFLOAT4(0.5f, 0.5f, 0.5, 1.f);
	m_light_data.MaterialSpecular = XMFLOAT4(0.5f, 0.5f, 0.5f, 1.f);
	...
	Device::GetInstance()->GetPtrImmContext()->
		UpdateSubresource(
			...
			GetPtr(ConstShader::PSType::VARIOUS_LIGHT)
			->GetConstBuffer(0),
			0,
			NULL,
			&m_light_data,
